    ...
    <div className="mb-4">
      <div className="flex items-center gap-2">
        <div className="relative grow">
          <input
            ...
            className="w-full rounded border border-gray-700 p-1 pl-8"
          />
          <FaSearch className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
        </div>
        <button
          onClick={handleSearch}
          className="whitespace-nowrap rounded bg-blue-500 px-3 py-1 text-white"
        >
          検索
        </button>
        <button
          onClick={() => setIsAdvancedSearchVisible(!isAdvancedSearchVisible)}
          className="whitespace-nowrap rounded bg-gray-500 px-3 py-1 text-white"
        >
          詳細検索
        </button>
      </div>
      {isAdvancedSearchVisible && (
        <div className="mt-2 rounded bg-gray-100 p-2">
          <div className="mt-4 grid grid-cols-[1fr_4fr] gap-4">
            <div className="flex items-center">
              <label className="mr-2 w-20">カテゴリ</label>
            </div>
            <div>
              {allCategories.map((category) => (
                <div key={category} className="mr-2 inline-block">
                  <label>
                    <input
                      type="checkbox"
                      value={category}
            ...
            <div className="relative flex items-center">
              ...
              />
              <FaCalendarAlt className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
            </div>
            <hr className="col-span-2 my-2 border-gray-400" />
            <div className="flex items-center">
